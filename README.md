...
To get started, navigate to **Connectors → Apify** in the Cinatra settings UI, paste your API token (find it at `console.apify.com → Administration → API & Integrations`), and click **Save & register MCP**. Nango must be configured first (`/configuration/llm/nango`); the setup page shows a clear warning if it is not. To revoke access, click **Disconnect** — this removes the stored credential and deregisters the MCP server.

If the MCP server stops responding, check that your Apify API token is still valid and re-save the connection. If the setup page shows "Nango is not configured", configure Nango before reconnecting.
